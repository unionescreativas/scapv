        ...
        <b-col sm="12" lg="12">
            <div class="iq-card">
                <ValidationObserver tag="form" ref="observer" autocomplete="off">
                    <form-wizard ref="formWizard" title="Por favor Completar todos los Campos" subtitle back-button-text="Atrás" next-button-text="Siguiente" finish-button-text="Finalizar" color="#083696" :start-index.sync="stepIndex">
                        <tab-content title="Datos Personales" icon="fa fa-search">
                            <template v-if="stepIndex == 0">
                                <ValidationProvider name="tipo_documento" v-slot="{ errors }">
                                    <b-form-group label="TIPO DE DOCUMENTO: *">
                                        <v-select placeholder="Seleccione ..." :options="options.tipos_documentos" label="valor_campo_1" :reduce="(option) => option.codigo_campo" v-model="form.tipo_documento">
                                            <template slot="no-options">
                                                <span>Escriba para buscar ...</span>
                                            </template>
                                        </v-select>
                                        <div style="color: var(--iq-danger-light);" v-if="errors[0]">{{ errors[0].replace("tipo documento", "") }}</div>
                                    </b-form-group>
                                </ValidationProvider>
                                ...
                                <ValidationProvider name="numero_documento" v-slot="{ errors }">
                                    <b-form-group label="NUMERO DE DOCUMENTO: *">
                                        ...
                                        <div style="color: var(--iq-danger-light);" v-if="errors[0]">{{ errors[0].replace("numero_documento", "") }}</div>
                                    </b-form-group>
                                </ValidationProvider>
                                ...
                                <ValidationProvider name="fecha_expedicion" v-slot="{ errors }">
                                    <b-form-group label="FECHA DE EXPEDICION DEL DOCUMENTO: *">
                                        <b-form-input type="date" placeholder="INGRESE FECHA DE EXPEDICION DEL DOCUMENTO" v-model="form.fecha_expedicion" />
                                        <div style="color: var(--iq-danger-light);" v-if="errors[0]">{{ errors[0].replace("fecha expedicion", "") }}</div>
                                    </b-form-group>
                                </ValidationProvider>

                                <ValidationProvider name="fecha_vencimiento" v-slot="{ errors }">
                                    <b-form-group label="FECHA DE VENCIMIENTO DEL DOCUMENTO: *">
                                        <b-form-input type="date" placeholder="INGRESE FECHA DE VENCIMIENTO DEL DOCUMENTO" v-model="form.fecha_vencimiento" />
                                        <div style="color: var(--iq-danger-light);" v-if="errors[0]">{{ errors[0].replace("fecha_vencimiento", "") }}</div>
                                    </b-form-group>
                                </ValidationProvider>
                                ...
                                <ValidationProvider name="fecha_nacimiento" v-slot="{ errors }">
                                    <b-form-group label="FECHA DE NACIMIENTO: *">
                                        ...
                                        <div style="color: var(--iq-danger-light);" v-if="errors[0]">{{ errors[0].replace("fecha nacimiento", "") }}</div>
                                    </b-form-group>
                                </ValidationProvider>
                                ...
                                <ValidationProvider name="genero" v-slot="{ errors }">
                                    <b-form-group label="GÉNERO: *">
                                        <v-select placeholder="Seleccione ..." :options="options.generos" label="valor_campo_1" :reduce="(option) => option.codigo_campo" v-model="form.genero">
                                            <template slot="no-options">
                                                <span>Escriba para buscar ...</span>
                                            </template>
                                        </v-select>
                                ...
                                <ValidationProvider name="estado_civil" v-slot="{ errors }">
                                    <b-form-group label="ESTADO CIVIL">
                                        <v-select placeholder="Seleccione ..." :options="options.estados_civiles" label="valor_campo_1" :reduce="(option) => option.codigo_campo" v-model="form.estado_civil">
                                            <template slot="no-options">
                                                <span>Escriba para buscar ...</span>
                                            </template>
                                        </v-select>
                                ...
                                <ValidationProvider name="ciudad" v-slot="{ errors }">
                                    <b-form-group label="CIUDAD DE RESIDENCIA">
                                        <v-select placeholder="Seleccione ..." :options="options.ciudades_origen" label="valor_campo_1" :reduce="(option) => option.codigo_campo" v-model="form.ciudad">
                                            <template slot="no-options">
                                                <span>Escriba para buscar ...</span>
                                            </template>
                                        </v-select>
                                ...
                                <ValidationProvider name="barrio" v-slot="{ errors }">
                                    <b-form-group label="BARRIO DE RESIDENCIA: *">
                                        <v-select ref="vSelect" placeholder="Seleccione ..." :options="options.barrios" label="valor_campo_1" :reduce="(option) => option.codigo_campo" v-model="form.barrio" @input="autocompletarComuna">
                                            <template slot="no-options">
                                                <span>Escriba para buscar ...</span>
                                            </template>
                                        </v-select>
                                ...
                                <ValidationProvider name="comuna" v-slot="{ errors }">
                                    <b-form-group label="COMUNA DE RESIDENCIA: *">
                                        <b-form-input ref="inputComuna" type="text" id="comuna" placeholder="INGRESE COMUNA DE RESIDENCIA" v-model="form.comuna" readonly />
                            ...
                            <template v-if="stepIndex == 1">
                                ...
                                <ValidationProvider name="ciudad_origen" v-slot="{ errors }">
                                    <b-form-group label="CIUDAD DE ORIGEN">
                                        <v-select placeholder="Seleccione ..." :options="options.ciudades_origen" label="valor_campo_1" :reduce="(option) => option.codigo_campo" v-model="form.ciudad_origen">
                                            <template slot="no-options">
                                                <span>Escriba para buscar ...</span>
                                            </template>
                                        </v-select>
                                ...
                                <ValidationProvider name="pais_origen" v-slot="{ errors }">
                                    <b-form-group label="PAIS DE ORIGEN">
                                        <v-select placeholder="Seleccione ..." :options="options.paises_origen" label="valor_campo_1" :reduce="(option) => option.codigo_campo" v-model="form.pais_origen">
                                            <template slot="no-options">
                                                <span>Escriba para buscar ...</span>
                                            </template>
                                        </v-select>
                                ...
                                <ValidationProvider name="respuesta_intencion" v-slot="{ errors }">
                                    <b-form-group label="RESPUESTA INTENCIÓN">
                                        ...
                                        <div style="color: var(--iq-danger-light);" v-if="errors[0]">{{ errors[0].replace("respuesta_intencion", "") }}</div>
                                    </b-form-group>
                                </ValidationProvider>

                                <ValidationProvider name="discapacidad" v-slot="{ errors }">
                                    <b-form-group label="TIENE ALGUNA DISCAPACIDAD">
                                        <v-select placeholder="Seleccione ..." :options="options.respuestas" label="valor_campo_1" :reduce="(option) => option.codigo_campo" v-model="form.discapacidad">
                                            <template slot="no-options">
                                                <span>Escriba para buscar ...</span>
                                            </template>
                                        </v-select>
                                ...
                                <ValidationProvider name="salud" v-slot="{ errors }">
                                    <b-form-group label="TIENE SISTEMA DE SALUD">
                                        <v-select placeholder="Seleccione ..." :options="options.respuestas" label="valor_campo_1" :reduce="(option) => option.codigo_campo" v-model="form.salud">
                                            <template slot="no-options">
                                                <span>Escriba para buscar ...</span>
                                            </template>
                                        </v-select>
                                ...
                                <ValidationProvider name="estudia_actualmente" v-slot="{ errors }">
                                    <b-form-group label="SE ENCUENTRA ESTUDIANDO ACTUALMENTE">
                                        <v-select placeholder="Seleccione ..." :options="options.respuestas" label="valor_campo_1" :reduce="(option) => option.codigo_campo" v-model="form.estudia_actualmente">
                                            <template slot="no-options">
                                                <span>Escriba para buscar ...</span>
                                            </template>
                                        </v-select>
                                        <div style="color: var(--iq-danger-light);" v-if="errors[0]">{{ errors[0].replace("estudia_actualmente", "") }}</div>
                                    </b-form-group>
                                </ValidationProvider>

                                <ValidationProvider name="nivel_escolaridad" v-slot="{ errors }">
                                    <b-form-group label="NIVEL DE ESCOLARIDAD">
                                        <v-select placeholder="Seleccione ..." :options="options.niveles_escolares" label="valor_campo_1" :reduce="(option) => option.codigo_campo" v-model="form.nivel_escolaridad">
                                            <template slot="no-options">
                                                <span>Escriba para buscar ...</span>
                                            </template>
                                        </v-select>
                                        <div style="color: var(--iq-danger-light);" v-if="errors[0]">{{ errors[0].replace("nivel_escolaridad", "") }}</div>
                                    </b-form-group>
                                </ValidationProvider>
                                ...
                                <ValidationProvider name="trabajo" v-slot="{ errors }">
                                    <b-form-group label="ACTUALMENTE SE ENCUENTRA LABORANDO">
                                        <v-select placeholder="Seleccione ..." :options="options.respuestas" label="valor_campo_1" :reduce="(option) => option.codigo_campo" v-model="form.trabajo">
                                            <template slot="no-options">
                                                <span>Escriba para buscar ...</span>
                                            </template>
                                        </v-select>
                                ...
                                <ValidationProvider name="tipo_empleo" v-slot="{ errors }">
                                    <b-form-group label="TIPO DE EMPLEO">
                                        <v-select placeholder="Seleccione ..." :options="options.tipos_empleos" label="valor_campo_1" :reduce="(option) => option.codigo_campo" v-model="form.tipo_empleo">
                                            <template slot="no-options">
                                                <span>Escriba para buscar ...</span>
                                            </template>
                                        </v-select>
                ...
                Yuhuuu! This seems pretty damn simple
                        ...
                        <template slot="footer" slot-scope="props">
                            <div class="wizard-footer-left">
                                <b-button v-if="props.activeTabIndex > 0 || props.isLastStep" @click="prevTab()" :style="props.fillButtonStyle">Atrás</b-button>
                            </div>
                            <div class="wizard-footer-right">
                                <b-button v-if="!props.isLastStep" @click="validate(() => guardarCiudadano(_self))" class="wizard-footer-right" :style="props.fillButtonStyle">Siguiente</b-button>
                                <b-button v-if="props.isLastStep" @click="validate(() => guardarCiudadano(_self))" class="wicozard-footer-right finish-button" :style="props.fillButtonStyle">Finalizar</b-button>
                            </div>
                        </template>
                    </form-wizard>
                </ValidationObserver>
...
export default {
    ...
    methods: {
        ...
        autocompletarComuna(value) {
            let barrio = this.options.barrios.find(
                (option) => option.codigo_campo == value
            );
            let comuna = barrio.valor_campo_2;
            this.$refs.inputComuna._data.localValue = comuna;
            this.form.comuna = comuna;
        },
        calcularEdad(fecha) {
            let edad = calcularEdad(fecha);
            // console.log(this.$refs.inputEdad);
            this.form.edad = edad;
        ...
        async validate(func) {
            try {
                ...
                let errors = _.pick(
                    ...
                    _.keys(this.$refs.observer.fields)
                );
    ...
    created() {
        this.form = {
            ...
            ...(this.ciudadano && this.ciudadano != "no existe" ?
                this.ciudadano[0] :
                null),
        };
        ...
        Object.keys(this.options).forEach((option) => {
            this.$axios
                .get(`/api/listas/${option}`)
                .then((res) => (this.options[option] = res.data.data));
        });
    },
};
</script>
