  ...
  methods: {
    ...
    async consultarCiudadano() {
      try {
        ...
        if (validacion) {
          consultaAyudas = await this.$axios.get(
            `/api/consultandounidades/${this.ciudadano}/${this.ayuda}/${this.cantidad}`
          );
          if (consultaAyudas.data.validacion) {
            this.validarCargarAyuda = true;
          } else {
            Swal.fire({
              html: `<h4>Ya existe un registro de ayudas para este Nucleo Familiar el día : ${consultaAyudas.data.data[0].fecha_entrega} !</h4>`,
              icon: "warning",
              allowOutsideClick: false,
              allowEscapeKey: false,
            });
